...
const holidayCalendarsStore: unknown[] = [
  { calendarId: 1, calendarCode: 'UK_BANK', calendarName: 'UK Bank Holidays', calendarType: 'BANKING', countryCode: 'GB', currencyCode: 'GBP', description: 'UK public holidays and Bank Holidays as observed by the London interbank market', isActive: true, holidayCount: 82, createdAt: '2024-01-01T00:00:00Z' },
  { calendarId: 2, calendarCode: 'US_FEDERAL', calendarName: 'US Federal Holidays', calendarType: 'BANKING', countryCode: 'US', currencyCode: 'USD', description: 'US Federal Reserve banking holidays — SIFMA calendar basis', isActive: true, holidayCount: 80, createdAt: '2024-01-01T00:00:00Z' },
  { calendarId: 3, calendarCode: 'NYMEX_WTI', calendarName: 'NYMEX WTI Futures Calendar', calendarType: 'EXCHANGE', countryCode: 'US', currencyCode: 'USD', description: 'CME/NYMEX exchange trading holidays', isActive: true, holidayCount: 65, createdAt: '2024-01-01T00:00:00Z' },
  { calendarId: 4, calendarCode: 'LME_METALS', calendarName: 'LME Metals Calendar', calendarType: 'COMMODITY', countryCode: 'GB', currencyCode: 'USD', description: 'LME ring trading holidays and closure dates', isActive: true, holidayCount: 70, createdAt: '2024-01-01T00:00:00Z' },
  { calendarId: 5, calendarCode: 'ECB_TARGET', calendarName: 'ECB TARGET2 Payment System Holidays', calendarType: 'BANKING', countryCode: null, currencyCode: 'EUR', description: 'European Central Bank TARGET2 payment system holidays — applies to EUR settlements', isActive: true, holidayCount: 58, createdAt: '2024-01-01T00:00:00Z' },
  { calendarId: 6, calendarCode: 'ICE_BRENT', calendarName: 'ICE Brent Futures Calendar', calendarType: 'EXCHANGE', countryCode: 'GB', currencyCode: 'USD', description: 'ICE Futures Europe exchange holidays — Brent, Gasoil, TTF', isActive: true, holidayCount: 66, createdAt: '2024-01-01T00:00:00Z' },
];
...
function computeCreditLimit(input: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    cpCountryCode: (input['cpCountryCode'] as string | null) ?? (cp?.['jurisdiction'] as string | undefined) ?? null,
...
const tradeCostsStore: unknown[] = [
  { costId: 1, tradeId: 1, costType: 'LEGAL_DOCUMENTATION', description: 'GTC amendment review — EFET-OIL-2007 side letter', amount: 1500.00, currencyCode: 'USD', isEstimated: false, notes: 'Outside counsel review, invoiced' },
  { costId: 2, tradeId: 1, costType: 'BANK_CHARGES',        description: 'Letter of credit issuance fee',                    amount: 850.00,   currencyCode: 'USD', isEstimated: true,  notes: 'Estimated — LC not yet drawn' },
];

// ─── LEG COSTS (order-level secondary costs, V88) ─────────────────────────────
let tradeOrderCostIdSeq = 10;
const tradeOrderCostsStore: unknown[] = [
  { costId: 1, orderId: 1, costType: 'PORT_DUES',         description: 'Sullom Voe loading dues — NORDIC LUNA', amount: 28000.00, currencyCode: 'USD', isEstimated: false, notes: 'Actual — Sullom Voe terminal invoice' },
  { costId: 2, orderId: 1, costType: 'INSPECTION_SURVEY', description: 'Independent cargo inspection (SGS)',     amount: 6200.00,  currencyCode: 'USD', isEstimated: true,  notes: 'Estimated pending SGS invoice' },
];
...
export const etrmHandlers = [
  ...
  http.post(`${API}/trade-costs`, async ({ request }) => {
    const input = (await request.json()) as Record<string, unknown>;
    const row = { ...input, costId: tradeCostIdSeq++ };
  ...
  http.put(`${API}/trade-costs/:id`, async ({ params, request }) => {
    ...
    const input = (await request.json()) as Record<string, unknown>;
    (tradeCostsStore as Array<Record<string, unknown>>)[idx] = { ...(tradeCostsStore as Array<Record<string, unknown>>)[idx], ...input };
  ...
  http.post(`${API}/trade-order-costs`, async ({ request }) => {
    const input = (await request.json()) as Record<string, unknown>;
    const row = { ...input, costId: tradeOrderCostIdSeq++ };
  ...
  http.put(`${API}/trade-order-costs/:id`, async ({ params, request }) => {
    ...
    const input = (await request.json()) as Record<string, unknown>;
    (tradeOrderCostsStore as Array<Record<string, unknown>>)[idx] = { ...(tradeOrderCostsStore as Array<Record<string, unknown>>)[idx], ...input };
