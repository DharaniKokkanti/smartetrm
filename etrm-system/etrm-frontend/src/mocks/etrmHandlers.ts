...
function denormalizeOrder(input: Record<string, unknown>): Record<string, unknown> {
  const bookId = input['bookId'] as number | null;
  const leId = input['legalEntityId'] as number | null;
...
/** Resolves uomId → uomCode and currencyId → currencyCode for trade items, mirroring denormalizeOrder above. */
...
let tradeOrderCostIdSeq = 10;
const tradeOrderCostsStore: unknown[] = [
  { costId: 1, orderId: 1, costType: 'PORT_DUES',         description: 'Sullom Voe loading dues — NORDIC LUNA', amount: 28000.00, currencyId: 1, currencyCode: 'USD', isEstimated: false, notes: 'Actual — Sullom Voe terminal invoice' },
  { costId: 2, orderId: 1, costType: 'INSPECTION_SURVEY', description: 'Independent cargo inspection (SGS)',     amount: 6200.00,  currencyId: 1, currencyCode: 'USD', isEstimated: true,  notes: 'Estimated pending SGS invoice' },
];

// ─── ASSAY RESULTS (physical-leg quality results, V88) ────────────────────────
// Order 1's product (BRENT-CRUDE, productId=1) already has spec templateId=1
// (Dated Brent/BFOE Standard) whose API_GRAVITY/SULPHUR_PCT values (specValueId
// 1/2, see productSpecValueStore above) match the order's own
...
const assayResultsStore: unknown[] = [
  { assayResultId: 1, orderId: 1, specValueId: 1, actualValue: 40.7, actualText: null, samplePoint: 'DISCHARGE', recordedDate: '2026-07-12', notes: 'Certificate of Quality — Rotterdam discharge, within Brent 28-46 range' },
  { assayResultId: 2, orderId: 1, specValueId: 2, actualValue: 0.26, actualText: null, samplePoint: 'DISCHARGE', recordedDate: '2026-07-12', notes: 'Certificate of Quality — sweet crude, well within 0.60% max' },
];
...
const legCustomFieldValueStore: unknown[] = [
  { valueId: 1, orderId: 1, definitionId: 2, valueText: 'AFRAMAX',    valueNumber: null, valueDate: null, valueBoolean: null },
  { valueId: 2, orderId: 1, definitionId: 3, valueText: 'London-EU',  valueNumber: null, valueDate: null, valueBoolean: null },
];
...
function computeNomination(input: Record<string, unknown>): Record<string, unknown> {
  const order = (tradeOrdersStore as Array<Record<string, unknown>>).find((o) => o['orderId'] === input['orderId']);
...
function tradeOrderOptions(): Record<string, unknown>[] {
  return (tradeOrdersStore as Array<Record<string, unknown>>).map((o) => ({
    orderId: o['orderId'], orderReference: o['orderReference'],
  }));
}
...
export const etrmHandlers = [
  ...
  http.post(`${API}/trade-orders`, async ({ request }) => {
    ...
    const row = { ...denormalizeOrder(input), orderId: orderIdSeq++, orderSequence: nextSeq, orderReference: ref, createdAt: now(), updatedAt: now() };
  ...
  http.put(`${API}/trade-orders/:id`, async ({ params, request }) => {
    ...
    const input = (await request.json()) as Record<string, unknown>;
    (tradeOrdersStore as Array<Record<string, unknown>>)[idx] = { ...(tradeOrdersStore as Array<Record<string, unknown>>)[idx], ...denormalizeOrder(input), updatedAt: now() };
  ...
  // Leg Costs (order-level secondary costs, V88)
  http.get(`${API}/trade-order-costs`, ({ request }) => {
    const orderId = Number(new URL(request.url).searchParams.get('orderId'));
    const rows = (tradeOrderCostsStore as Array<Record<string, unknown>>).filter((c) => c['orderId'] === orderId);
    return HttpResponse.json(rows);
  }),
  http.post(`${API}/trade-order-costs`, async ({ request }) => {
    const input = (await request.json()) as Record<string, unknown>;
    const row = { ...denormalizeCost(input), costId: tradeOrderCostIdSeq++ };
    tradeOrderCostsStore.push(row);
    return HttpResponse.json(row, { status: 201 });
  }),
  http.put(`${API}/trade-order-costs/:id`, async ({ params, request }) => {
    const idx = (tradeOrderCostsStore as Array<Record<string, unknown>>).findIndex((c) => c['costId'] === Number(params.id));
    if (idx === -1) return problem(404, 'Not Found', `Leg cost ${params.id} not found.`);
    const input = (await request.json()) as Record<string, unknown>;
    (tradeOrderCostsStore as Array<Record<string, unknown>>)[idx] = { ...(tradeOrderCostsStore as Array<Record<string, unknown>>)[idx], ...denormalizeCost(input) };
    return HttpResponse.json((tradeOrderCostsStore as Array<Record<string, unknown>>)[idx]);
  }),
  http.delete(`${API}/trade-order-costs/:id`, ({ params }) => {
    const idx = (tradeOrderCostsStore as Array<Record<string, unknown>>).findIndex((c) => c['costId'] === Number(params.id));
    if (idx === -1) return problem(404, 'Not Found', `Leg cost ${params.id} not found.`);
    tradeOrderCostsStore.splice(idx, 1);
    return new HttpResponse(null, { status: 204 });
  }),

  // Assay Results (physical-leg quality results, V88)
  http.get(`${API}/trade-order-assay-results`, ({ request }) => {
    const orderId = Number(new URL(request.url).searchParams.get('orderId'));
    const rows = (assayResultsStore as Array<Record<string, unknown>>).filter((a) => a['orderId'] === orderId).map(joinAssayResult);
    return HttpResponse.json(rows);
  }),
  http.post(`${API}/trade-order-assay-results`, async ({ request }) => {
    const input = (await request.json()) as Record<string, unknown>;
  ...
  http.put(`${API}/trade-order-assay-results/:id`, async ({ params, request }) => {
  ...
  http.delete(`${API}/trade-order-assay-results/:id`, ({ params }) => {
    const idx = (assayResultsStore as Array<Record<string, unknown>>).findIndex((a) => a['assayResultId'] === Number(params.id));
    if (idx === -1) return problem(404, 'Not Found', `Assay result ${params.id} not found.`);
  ...
  // Leg-level custom field values (V89) — upsert semantics: one row per (orderId, definitionId)
  http.get(`${API}/trade-order-custom-field-values`, ({ request }) => {
    const orderId = Number(new URL(request.url).searchParams.get('orderId'));
    const rows = (legCustomFieldValueStore as Array<Record<string, unknown>>).filter((v) => v['orderId'] === orderId);
    return HttpResponse.json(rows);
  }),
  http.post(`${API}/trade-order-custom-field-values`, async ({ request }) => {
    const input = (await request.json()) as Record<string, unknown>;
    const idx = (legCustomFieldValueStore as Array<Record<string, unknown>>).findIndex(
      (v) => v['orderId'] === input['orderId'] && v['definitionId'] === input['definitionId'],
    );
    if (idx !== -1) {
  ...
  http.delete(`${API}/trade-order-custom-field-values/:id`, ({ params }) => {
