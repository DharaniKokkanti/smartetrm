...
export function FieldHint({ label, hint, example, format, required }: FieldHintProps) {
  const content = (
    <div style={{ maxWidth: 280 }}>
      {/* This tooltip is deliberately always-dark (color="#1a1a2e" below) regardless
          of the app's light/dark theme toggle, so its internal text tints are fixed
          white/near-white values, not theme tokens — using color.textPrimary etc.
          here would flip to dark text on a dark tooltip in light mode. */}
      ...
      {format && (
        <div style={{ marginTop: 6 }}>
          <Typography.Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11 }}>
            {/* #7EC8E3: light blue chosen for contrast on the always-dark tooltip bg, not a theme token */}
      ...
      {example && (
        <div style={{ marginTop: 4 }}>
          <Typography.Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11 }}>
            {/* #98E098: light green chosen for contrast on the always-dark tooltip bg, not a theme token */}
  ...
  return (
    <Space size={4} align="center">
      {required && <span style={{ color: color.error, fontSize: 12, lineHeight: 1 }}>*</span>}
      <span>{label}</span>
      {/* #1a1a2e: deliberately fixed dark tooltip surface, independent of app theme */}
