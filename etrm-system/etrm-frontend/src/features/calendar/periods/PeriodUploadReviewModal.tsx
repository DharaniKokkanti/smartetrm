...
import type { PeriodUploadRow } from './types';
import { useBulkCreatePeriods } from './hooks';

interface Props {
  open: boolean;
  rows: PeriodUploadRow[];
  onClose: () => void;
}

export function PeriodUploadReviewModal({ open, rows, onClose }: Props) {
  const { message } = AntApp.useApp();
  const bulkCreate = useBulkCreatePeriods();
  ...
  async function handleConfirm() {
    ...
    message.success(
      `Imported ${result.created.length} period(s).` + (result.rejected.length ? ` ${result.rejected.length} rejected by server.` : ''),
    );
    onClose();
  }
            ...
            children: (
              <Table
                ...
                columns={[
                  ...
                  { title: 'Code', dataIndex: 'periodCode' },
                  { title: 'Name', dataIndex: 'periodName' },
                  { title: 'Type', dataIndex: 'periodType' },
