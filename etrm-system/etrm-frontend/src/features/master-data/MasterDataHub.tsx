...
const ALL_ENTRIES: Entry[] = [
  ...
  { live: true,  db: 'app_user',       group: 'Organization & Users', label: 'System Users',     path: '/admin/users',        description: 'Login accounts with assigned roles — ADMIN, TRADER, RISK_MANAGER, OPERATIONS, COMPLIANCE, VIEWER.', tags: ['user', 'role', 'access', 'admin', 'permission'] },
  ...
  { live: true,  db: 'desk',           group: 'Organization & Users', label: 'Trading Desks',    path: '/org/desks',          description: 'Commodity desk hierarchy — crude, gas, power, metals. Defines commodity type assignments and desk heads.', tags: ['desk', 'trading', 'commodity', 'team'] },
  { live: true,  db: 'book',           group: 'Organization & Users', label: 'Trading Books',    path: '/org/books',          description: 'P&L books with position limits, VAR limits, and desk assignment. Each trade is booked to a book.', tags: ['book', 'pnl', 'var', 'position', 'limit'] },
  { live: true,  db: 'trader',         group: 'Organization & Users', label: 'Traders',          path: '/org/traders',        description: 'Individual traders — single-trade limits, daily limits, commodity mandates, and approval chains.', tags: ['trader', 'limit', 'mandate', 'approval'] },
  { live: false, db: 'contact',        group: 'Organization & Users', label: 'Contacts',         path: '/org/contacts',       description: 'Named contacts at counterparties and legal entities — ops, credit, trading, compliance contacts.', tags: ['contact', 'person', 'ops', 'credit', 'trading'] },
  ...
  { live: false, db: 'netting_agreement',    group: 'Counterparties & Agreements', label: 'Netting Agreements',      path: '/counterparties/netting',         description: 'ISDA/EFET master netting agreements. Determines whether offsetting trades with a counterparty can be netted for settlement.', tags: ['netting', 'isda', 'efet', 'master agreement', 'set-off'] },
  { live: false, db: 'cp_commercial_terms',  group: 'Counterparties & Agreements', label: 'CP Commercial Terms',     path: '/counterparties/commercial-terms', description: 'Default payment terms, settlement days, and currency preferences per counterparty–legal entity pair.', tags: ['commercial', 'terms', 'default', 'settlement', 'payment'] },
  { live: false, db: 'cp_gtc_agreement',     group: 'Counterparties & Agreements', label: 'CP GTC Agreements',       path: '/counterparties/gtc-agreements',  description: 'Which version of which GTC is agreed per counterparty and legal entity — the contractual foundation for each relationship.', tags: ['gtc', 'agreement', 'contract', 'efet', 'isda', 'gafta'] },

  // ── Credit & Collateral ───────────────────────────────────────
  { live: false, db: 'bank_guarantee',        group: 'Credit & Collateral', label: 'Bank Guarantees',         path: '/credit/bank-guarantees',   description: 'Performance and payment bank guarantees issued or received — guarantor bank, amount, expiry, commodity context.', tags: ['bank guarantee', 'bg', 'performance', 'payment', 'guarantor'] },
  { live: false, db: 'letter_of_credit',      group: 'Credit & Collateral', label: 'Letters of Credit',       path: '/credit/letters-of-credit', description: 'Irrevocable/revolving/standby LCs — issuing bank, beneficiary, LC amount, tolling date, and amendment tracking.', tags: ['lc', 'letter of credit', 'standby', 'revolving', 'issuing bank'] },
  { live: false, db: 'parent_company_guarantee', group: 'Credit & Collateral', label: 'Parent Company Guarantees', path: '/credit/pcg',            description: 'PCGs from parent companies guaranteeing counterparty obligations — essential for intercompany and subsidiary trading.', tags: ['pcg', 'parent', 'guarantee', 'subsidiary', 'intercompany'] },
  { live: false, db: 'collateral',            group: 'Credit & Collateral', label: 'Collateral',              path: '/credit/collateral',        description: 'Cash and non-cash collateral posted or received — linked to margin accounts, direction, and value dates.', tags: ['collateral', 'margin', 'cash', 'securities', 'initial margin', 'variation margin'] },
  { live: false, db: 'margin_account',        group: 'Credit & Collateral', label: 'Margin Accounts',         path: '/credit/margin-accounts',   description: 'Exchange margin accounts for cleared trades — initial and variation margin tracking per exchange.', tags: ['margin', 'initial margin', 'variation', 'exchange', 'cleared', 'ccp'] },
  { live: false, db: 'insurance_policy',      group: 'Credit & Collateral', label: 'Insurance Policies',      path: '/credit/insurance',         description: 'Cargo, credit, and political risk insurance policies — insurer, coverage period, insured values per commodity.', tags: ['insurance', 'cargo', 'credit risk', 'political risk', 'marine', 'policy'] },
  { live: false, db: 'insurance_provider',    group: 'Credit & Collateral', label: 'Insurance Providers',     path: '/credit/insurance-providers', description: 'Insurance companies — Lloyd\'s syndicates, AIG, Zurich, Euler Hermes — with contact and credit rating.', tags: ['insurer', 'lloyds', 'aig', 'zurich', 'euler hermes', 'provider'] },
  { live: false, db: 'tax_registration',      group: 'Credit & Collateral', label: 'Tax Registrations',       path: '/credit/tax',               description: 'VAT, GST, and sales tax registration numbers per legal entity and counterparty per jurisdiction.', tags: ['tax', 'vat', 'gst', 'registration', 'jurisdiction', 'tin'] },
  ...
  // ── Logistics & Delivery ──────────────────────────────────────
  { live: true,  db: 'location',         group: 'Logistics & Delivery', label: 'Locations',           path: '/logistics/locations',      description: 'Ports, gas hubs, grid nodes, LNG terminals, refinery gates, pipeline IPs. Used as delivery/loading points on trades.', tags: ['location', 'port', 'hub', 'terminal', 'cushing', 'ttf', 'nbp', 'rotterdam', 'sullom voe'] },
  ...
  { live: true,  db: 'vessel',           group: 'Logistics & Delivery', label: 'Vessels',             path: '/logistics/vessels',        description: 'Tankers with IMO number, vessel class, SIRE inspection expiry, CDI vetting, DWT, and cargo restrictions.', tags: ['vessel', 'ship', 'tanker', 'vlcc', 'aframax', 'suezmax', 'imo', 'sire', 'cdi', 'vetting', 'dwt'] },
  { live: false, db: 'vessel_certificate', group: 'Logistics & Delivery', label: 'Vessel Certificates', path: '/logistics/vessel-certs',   description: 'SOLAS, MARPOL, ISM, ISSC, class society certificates with expiry dates tracked per vessel.', tags: ['certificate', 'solas', 'marpol', 'ism', 'issc', 'class society', 'lloyds', 'dnv', 'bureau veritas'] },
  ...
  { live: true,  db: 'pipeline',         group: 'Logistics & Delivery', label: 'Pipelines',           path: '/logistics/pipelines',      description: 'Oil and gas pipeline systems — TSO, regulatory body, capacity, tariff, and product approvals.', tags: ['pipeline', 'tso', 'crude', 'gas', 'druzhba', 'keystone', 'ferc', 'ofgem', 'capacity'] },
  { live: false, db: 'pipeline_segment', group: 'Logistics & Delivery', label: 'Pipeline Segments',   path: '/logistics/pipeline-segments', description: 'Individual pipeline segments with injection/offtake points, flow direction, and nomination cycles.', tags: ['segment', 'injection', 'offtake', 'nomination', 'cycle', 'capacity'] },
  ...
  { live: true,  db: 'truck',            group: 'Logistics & Delivery', label: 'Trucks & Road',       path: '/logistics/trucks',         description: 'Road tankers and bulk vehicles — ADR hazmat certificate, GVW, inspection expiry, operator, license plate.', tags: ['truck', 'tanker', 'road', 'adr', 'gvw', 'license', 'isotank', 'bulk'] },
  { live: true,  db: 'storage_facility', group: 'Logistics & Delivery', label: 'Storage Facilities',  path: '/logistics/storage',        description: 'Tank farms, gas caverns, LNG tanks, LME warehouses — injection/withdrawal rates, regulatory ref, operator.', tags: ['storage', 'tank', 'cavern', 'lme', 'warehouse', 'lng', 'gas', 'injection', 'withdrawal'] },
  { live: false, db: 'tank',             group: 'Logistics & Delivery', label: 'Tanks',               path: '/logistics/tanks',          description: 'Individual storage tanks within a facility — calibration tables, capacity, product grade, current status.', tags: ['tank', 'capacity', 'calibration', 'ullage', 'product', 'grade'] },
  { live: false, db: 'container',        group: 'Logistics & Delivery', label: 'Containers',          path: '/logistics/containers',     description: 'ISO containers and flexibags for bulk liquids and packaged goods — container number, type, owner, depot.', tags: ['container', 'iso', 'flexibag', '20ft', '40ft', 'depot', 'box'] },
  { live: false, db: 'railcar',          group: 'Logistics & Delivery', label: 'Rail Cars',           path: '/logistics/railcars',       description: 'Tank cars and bulk railcars — AAR designation, capacity, lessee, inspection date, commodity approval.', tags: ['railcar', 'tank car', 'aar', 'rail', 'bulk', 'lessee'] },
  ...
  { live: false, db: 'transport_operator', group: 'Logistics & Delivery', label: 'Transport Operators', path: '/logistics/operators',    description: 'Haulage companies, rail operators, barge operators — contact, coverage area, commodity approvals.', tags: ['operator', 'haulage', 'rail', 'barge', 'carrier'] },
  ...
  // ── Power & Energy ────────────────────────────────────────────
  { live: false, db: 'generation_asset',     group: 'Power & Energy', label: 'Generation Assets',     path: '/power/generation-assets',    description: 'Power plants — gas peakers, wind farms, nuclear — with installed capacity, location, and balancing authority.', tags: ['generation', 'power plant', 'gas peaker', 'wind', 'nuclear', 'solar', 'capacity', 'mw'] },
  ...
  { live: false, db: 'interconnector',      group: 'Power & Energy', label: 'Interconnectors',       path: '/power/interconnectors',      description: 'Cross-border and inter-zone power interconnectors — BritNed, IFA, Moyle — with capacity and regulatory auction rules.', tags: ['interconnector', 'britned', 'ifa', 'moyle', 'cross-border', 'capacity', 'auction'] },
  ...
  { live: false, db: 'bank_account',  group: 'Finance & Settlement', label: 'Bank Accounts',    path: '/finance/bank-accounts',   description: 'Settlement bank accounts for legal entities and counterparties — IBAN, BIC/SWIFT, currency, account type (SETTLEMENT/MARGIN).', tags: ['bank', 'account', 'iban', 'bic', 'swift', 'nostro', 'vostro', 'settlement', 'margin'] },
  ...
  { live: false, db: 'trade_repository',       group: 'Risk & Compliance', label: 'Trade Repositories',      path: '/compliance/trade-repos',   description: 'DTCC, REGIS-TR, ICE TVEL — registered trade repositories by regime with connection credentials and status.', tags: ['trade repository', 'dtcc', 'regis-tr', 'ice tvel', 'regulatory', 'reporting'] },
