...
**Status as of 2026-08-14**: `tran_` done for 32 tables (25 Trade Capture, V221; `rin_transaction`, V228; `pricing_dispute`/`pricing_event`, V235; `tank_status`, V238; `delivery_instruction`/`inspection`/`nomination`, V239; `margin_call`/`margin_valuation`/`credit_limit_alert`/`bg_amendment`/`lc_amendment`, V240). `ref_` done for 168 tables: 71 from earlier batches, plus Pricing & Rates (14, V235), Power & Energy (11, V236), 4 of 5 confirmed market/pricing leftovers (V237), Logistics & Delivery (18, V238), the real business tables inside "Organization & Users" (19, V239), and Credit & Collateral (15, V240 — 12 reclassified up from an initial mst_-by-allow-flag guess via real dedicated pages, plus `credit_limit_line_item` for a real nested-controller write path matching `trader_commodity_limit`'s pattern). Includes the platform's 4 most-referenced entities (`product`: 38 incoming FKs, `counterparty`/`legal_entity`: 34 each, `market`: 8) and `holiday_calendar`/`period` (25 combined incoming FKs). `mst_` — 56 tables total (24 from V227-V232, `pricing_type`/`pricing_window_rule` from V235, 3 Power & Energy tables from V236, 6 Logistics & Delivery tables from V238, 8 "Organization & Users" tables from V239, 10 Credit & Collateral tables from V240 — `collateral_type`/`credit_limit_status_type`/`credit_limit_type`/`governing_law_type`/`lc_status_type`/`lc_type`/`margin_agreement_type`/`valuation_frequency_type`/`tax_type`/`insurance_policy_coverage`). `usr_` — 9 tables (V233/V234). **Classification method, stable since V235/V236, reused unchanged through V240**: raw `allow_create/edit=0` is not sufficient evidence for `mst_` on its own — checked first whether a table has a real dedicated Tier1 page (or nested sub-resource) with working create/edit endpoints, and whether it's directly FK'd by a `tran_*` transaction-capture table — either signal overrides a stale lock flag toward `ref_`. **Real bug found and fixed as part of V240, not just naming**: `LcType.java` (`com.etrm.system.lookup.LcType`) has a real `@Table(name = "lc_type")` annotation that an earlier grep pass missed entirely (a shell/grep fluke, not a systematic gap like the `usr_`-batch's raw-SQL misses — a second, independent grep immediately after caught it) — backend failed to boot with `Schema-validation: missing table [lc_type]` until fixed to `mst_lc_type`; the fix and a full clean-boot verification landed in the same commit. `position`/`position_eod_snapshot`/`position_valuation` (computed P&L output, not reference data) deliberately left unprefixed. `dbo.trade_repository` (Sanctions & Regulatory Reporting) and `market_product` (orphaned registry row, no physical table) remain deliberately unprefixed. **All 6 module_groups originally identified as pending (Power & Energy, Pricing & Rates, Logistics & Delivery, Finance & Settlement, Voyage & Charter Ops, Credit & Collateral) are now done except Finance & Settlement and Voyage & Charter Ops** — see `ETRM_Project_Handoff_v1_0.md` §0 for the live batch-by-batch tracker and exact pending table lists.
