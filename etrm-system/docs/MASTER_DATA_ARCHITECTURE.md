...
**Status as of 2026-08-14**: `tran_` done for 40 tables (25 Trade Capture, V221; `rin_transaction`, V228; `pricing_dispute`/`pricing_event`, V235; `tank_status`, V238; `delivery_instruction`/`inspection`/`nomination`, V239; `margin_call`/`margin_valuation`/`credit_limit_alert`/`bg_amendment`/`lc_amendment`, V240; `bolmo_agreement`/`bolmo_leg`/`bunker_stem`/`charter_off_hire_event`/`charter_party`/`laytime_calculation`/`voyage_cargo_parcel`/`voyage_sof_event`, V242). `ref_` done for 197 tables: 71 from earlier batches, plus Pricing & Rates (14, V235), Power & Energy (11, V236), 4 of 5 confirmed market/pricing leftovers (V237), Logistics & Delivery (18, V238), the real business tables inside "Organization & Users" (19, V239), Credit & Collateral (15, V240), Finance & Settlement (15, V241), and Voyage & Charter Ops (14, V242 — 6 reclassified up from an initial mst_-by-allow-flag guess via real dedicated pages: `port_activity_template_step`/`vessel`/`vessel_cargo_tank`/`vessel_certificate`/`vessel_performance_curve`/`voyage`). Includes the platform's 4 most-referenced entities (`product`: 38 incoming FKs, `counterparty`/`legal_entity`: 34 each, `market`: 8) and `holiday_calendar`/`period` (25 combined incoming FKs). `mst_` — 57 tables total, unchanged by V242 (24 from V227-V232, `pricing_type`/`pricing_window_rule` from V235, 3 Power & Energy tables from V236, 6 Logistics & Delivery tables from V238, 8 "Organization & Users" tables from V239, 10 Credit & Collateral tables from V240, `uom_type` from V241). `usr_` — 9 tables (V233/V234). **Classification method, stable since V235/V236, reused unchanged through V242 — executed without a confirmation round in V242 per direct instruction ("don't ask my approval")**: raw `allow_create/edit=0` is not sufficient evidence for `mst_` on its own — checked first whether a table has a real dedicated Tier1 page (or nested sub-resource) with working create/edit endpoints, whether it's directly FK'd by a `tran_*` transaction-capture table, and whether `data_category=TRANSACTIONAL` (which always maps to `tran_` regardless of write-path presence, per every batch's precedent). **The V241 extra-verification step (re-grep the full table list a second time before touching the migration/frontend) is now permanent and caught another real miss in V242**: `VesselBunkerRobLedgerController` exists but is GET-only, and `vessel_bunker_rob_ledger` (an event-sourced, append-only ROB ledger, `DERIVED`, zero incoming FK consumers) was deliberately left unprefixed — the same computed-output shape as `position`/`position_eod_snapshot`/`position_valuation`, confirmed by the verification pass rather than assumed. `vessel` was also in `ReferenceDataTable.tsx`'s `DEDICATED_ENTITY_FK_TABLES` escape hatch — updated in the same commit, same as `location`/`storage_facility`/`transport_route`/`country`/`exchange`/`payment_term`/`unit_of_measure` in earlier batches. `dbo.trade_repository` (Sanctions & Regulatory Reporting) and `market_product` (orphaned registry row, no physical table) remain deliberately unprefixed. **All 7 module_groups originally identified as pending at the start of this session's continuation are now done.** See `ETRM_Project_Handoff_v1_0.md` §0 for the live batch-by-batch tracker and what's still genuinely open (the 9 reverted `usr_` tables, the `mst_` vendor-only enforcement gap, `market_product`'s orphaned row, and the rest of "Organization & Users" outside the business-table sweep already done).
