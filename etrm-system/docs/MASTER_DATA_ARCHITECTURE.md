...
**Status as of 2026-08-14**: `tran_` done for 29 tables (25 Trade Capture, V221; `rin_transaction`, V228; `pricing_dispute`/`pricing_event`, V235; `tank_status`, V238). `ref_` done for 134 tables: 71 from earlier batches, plus Pricing & Rates (14, V235, including the temporal `pricing_rule`+history pair), Power & Energy (11, V236, 2 reclassified up from an initial mst_ guess after checking real FK consumption by `tran_trade_power_detail`/`tran_trade_transmission_right_detail`), 4 of 5 confirmed market/pricing leftovers (V237 — `market_product`'s registry row is orphaned, no physical table exists, left untouched), and Logistics & Delivery (18, V238 — 11 reclassified up from an initial mst_ guess: `container`/`location`/`pipeline`/`pipeline_segment`/`pipeline_tariff`/`pipeline_cycle`/`railcar`/`storage_facility`/`tank`/`transport_route`/`truck` all had real dedicated Tier1 pages with working POST/PUT despite `allow=0`, plus `pipeline_point` for direct `tran_trade_oil_detail` FK consumption). Includes the platform's 4 most-referenced entities (`product`: 38 incoming FKs, `counterparty`/`legal_entity`: 34 each, `market`: 8) and `holiday_calendar`/`period` (25 combined incoming FKs). `mst_` — 38 tables total (24 from V227-V232, `pricing_type`/`pricing_window_rule` from V235, 3 Power & Energy tables from V236, 6 Logistics & Delivery tables from V238 — `location_type`/`mot_type`/`storage_facility_type`/`inspection_type`/`pipeline_point_product`/`pipeline_segment_product`). `usr_` — 9 tables (V233/V234). **Classification method, stable since V235/V236, reused unchanged for V238**: raw `allow_create/edit=0` is not sufficient evidence for `mst_` on its own — checked first whether a table has a real dedicated Tier1 page with working create/edit endpoints, and whether it's directly FK'd by a `tran_*` transaction-capture table — either signal overrides a stale lock flag toward `ref_`. `dbo.trade_repository` (Sanctions & Regulatory Reporting) and `market_product` (orphaned registry row, no physical table) remain deliberately unprefixed. See `ETRM_Project_Handoff_v1_0.md` §0 for the live batch-by-batch tracker and exact pending table lists (~220 tables remain across the untouched module_groups, plus the 9 reverted-pending-classification tables from V234 and the rest of "Organization & Users").
