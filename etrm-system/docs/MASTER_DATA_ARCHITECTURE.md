...
**Status as of 2026-08-14**: `tran_` done for all 25 Trade Capture tables (V221). `ref_` done for 62 tables across Supply & Distribution, Counterparties & Agreements, and Products & Markets (V222/V224/V225), including the platform's 4 most-referenced entities (`product`: 38 incoming FKs, `counterparty`/`legal_entity`: 34 each — both also system-versioned temporal tables, `market`: 8). `mst_` and `usr_` not yet started — see `ETRM_Project_Handoff_v1_0.md` §0 for the live batch-by-batch tracker and exact pending table lists (~390 tables remain across both, `module_group`'s "Organization & Users" bucket in particular needs a three-way `ref_`/`usr_`/`tran_` split, not a single pass, since it's the same kind of unaudited catch-all that caused the original drift).
...
- `ref_legal_entity_history`'s `module_group` is still `Organization & Users` (only `legal_entity` itself was corrected to `Counterparties & Agreements` in V223) — small, cheap, not yet fixed.
