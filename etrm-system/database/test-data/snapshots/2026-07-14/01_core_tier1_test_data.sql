...
--   market, price_index, user_role_assignment (new in this snapshot — see below)
--
-- 2026-07-14 snapshot — corrects a real bug in 2026-07-12's app_user rows:
-- password_hash was a literal placeholder string, not a real bcrypt hash,
-- so login against a real backend never actually worked with that data.
-- Fixed here (new dated snapshot, not an edit to 2026-07-12 — see this
-- folder's own README.md for why) by replacing it with a real BCrypt hash
-- of 'DevPassword123!' (same password as the mock-mode dev.admin shortcut,
-- so one password covers both). Also added a 5th app_user ('admin',
-- user_id 5) as the designated system-owner/super-user account, plus one
-- dbo.user_role_assignment row granting it the ADMIN role (role_id 1) —
-- see the comment above that INSERT for why it's pre-approved and what it
-- does/doesn't do today.
--
...
INSERT INTO dbo.app_user (user_id, legal_entity_id, username, email, password_hash, full_name, display_name, department, default_timezone, mfa_enabled, is_active, created_at, created_by, updated_at, updated_by) VALUES (1, 1, 'j.smith', 'j.smith@meridiantrading.example', '$2b$10$3D4UXXzE2j4u.oiQrNxY3u427qotGpAme5T1wCcocAgeZWVVAjPOK', 'John Smith', 'John Smith', 'Crude Oil Trading', 'Europe/London', 0, 1, SYSUTCDATETIME(), 'SYSTEM', SYSUTCDATETIME(), 'SYSTEM');
IF NOT EXISTS (SELECT 1 FROM dbo.app_user WHERE user_id = 2)
INSERT INTO dbo.app_user (user_id, legal_entity_id, username, email, password_hash, full_name, display_name, department, default_timezone, mfa_enabled, is_active, created_at, created_by, updated_at, updated_by) VALUES (2, 3, 'a.chen', 'a.chen@meridiantrading.example', '$2b$10$3D4UXXzE2j4u.oiQrNxY3u427qotGpAme5T1wCcocAgeZWVVAjPOK', 'Amy Chen', 'Amy Chen', 'Metals Trading', 'Asia/Singapore', 0, 1, SYSUTCDATETIME(), 'SYSTEM', SYSUTCDATETIME(), 'SYSTEM');
IF NOT EXISTS (SELECT 1 FROM dbo.app_user WHERE user_id = 3)
INSERT INTO dbo.app_user (user_id, legal_entity_id, username, email, password_hash, full_name, display_name, department, default_timezone, mfa_enabled, is_active, created_at, created_by, updated_at, updated_by) VALUES (3, 2, 'r.patel', 'r.patel@meridiantrading.example', '$2b$10$3D4UXXzE2j4u.oiQrNxY3u427qotGpAme5T1wCcocAgeZWVVAjPOK', 'Raj Patel', 'Raj Patel', 'Power & Gas Trading', 'America/New_York', 0, 1, SYSUTCDATETIME(), 'SYSTEM', SYSUTCDATETIME(), 'SYSTEM');
IF NOT EXISTS (SELECT 1 FROM dbo.app_user WHERE user_id = 4)
INSERT INTO dbo.app_user (user_id, legal_entity_id, username, email, password_hash, full_name, display_name, department, default_timezone, mfa_enabled, is_active, created_at, created_by, updated_at, updated_by) VALUES (4, 1, 'm.jones', 'm.jones@meridiantrading.example', '$2b$10$3D4UXXzE2j4u.oiQrNxY3u427qotGpAme5T1wCcocAgeZWVVAjPOK', 'Maria Jones', 'Maria Jones', 'Middle Office', 'Europe/London', 0, 1, SYSUTCDATETIME(), 'SYSTEM', SYSUTCDATETIME(), 'SYSTEM');
-- System owner / super-user account — the one intended for initial admin
-- access (create legal entities, desks, counterparties, and provision
-- further users from there), not a trading desk seat like users 1-4 above.
IF NOT EXISTS (SELECT 1 FROM dbo.app_user WHERE user_id = 5)
INSERT INTO dbo.app_user (user_id, legal_entity_id, username, email, password_hash, full_name, display_name, department, default_timezone, mfa_enabled, is_active, created_at, created_by, updated_at, updated_by) VALUES (5, 1, 'admin', 'admin@meridiantrading.example', '$2b$10$3D4UXXzE2j4u.oiQrNxY3u427qotGpAme5T1wCcocAgeZWVVAjPOK', 'System Administrator', 'Admin', 'IT Administration', 'Europe/London', 0, 1, SYSUTCDATETIME(), 'SYSTEM', SYSUTCDATETIME(), 'SYSTEM');
SET IDENTITY_INSERT dbo.app_user OFF;
GO

-- ============================================================
-- 2a. user_role_assignment — grants the 'admin' account (user_id 5) the
-- ADMIN role (role_id 1, seeded in V20/dbo.user_role) so it is the
-- designated super-user account. Pre-approved (status ACTIVE) rather than
-- left PENDING_APPROVAL, since this is the account that would otherwise
-- need to approve itself. Real per-role authorization enforcement is not
-- wired into the API yet (see AuthController's own doc comment — every
-- login currently gets a single ROLE_USER Spring Security authority
-- regardless of this table), so this only makes the row correct/complete
-- ahead of that landing, not a functional access change today.
-- ============================================================
SET IDENTITY_INSERT dbo.user_role_assignment ON;
IF NOT EXISTS (SELECT 1 FROM dbo.user_role_assignment WHERE assignment_id = 1)
INSERT INTO dbo.user_role_assignment (assignment_id, user_id, role_id, status, assigned_by, assigned_at, approved_by, approved_at, valid_from, is_active)
VALUES (1, 5, 1, 'ACTIVE', 'SYSTEM', SYSUTCDATETIME(), 'SYSTEM', SYSUTCDATETIME(), CAST(SYSUTCDATETIME() AS DATE), 1);
SET IDENTITY_INSERT dbo.user_role_assignment OFF;
GO
