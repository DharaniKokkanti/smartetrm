...
public class Nomination {
    ...
    // Setter exists only so NominationService.update() can copy the existing
    // value forward on the response object (mirrors createdAt above) — the
    // created_src_id column itself is updatable = false, so JPA
    // never re-writes it.
